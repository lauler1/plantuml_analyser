...
def allocate_an_address_on_border(highway_map: dict, comp_path: str, face: Dir, ranges: list[tuple[int, int]]) -> int|None:
    """
    ...
    for free_range in ranges:
        if free_range[1] - free_range[0] > 5: # Do not use too short ranges
            ...
            for offset in range (init_off, int(free_range[1]), step):
                if is_border_offset_available(highway_map, comp_path, face, offset):
...
def is_road_lane_available(highway_map: dict, road_name: str, lane: int) -> bool:
    ...
    allocarions = local_roads_map["allocations"][road_name]
    if lane in allocarions:
        print(f'  FALSE', allocarions)
        return False
...
def allocate_the_road_lane(highway_map: dict, road_name: str, lane: int):
    ...
    # if road_name in local_roads_map:
    allocarions = local_roads_map["allocations"][road_name]
    allocarions.append(lane)

def allocate_a_road_lane(highway_map: dict, road: str, prev_allocation: tuple[str, int]|None = None) -> int:
    """
    Automatically allocates a lane in a road. Multiple lanes are used to avoi passing one connection over other.
    highway_map: map generated by do_svg_architecture.recurrent_layout_sizing. All coordinates are in pixels in the svg context.
    ...
    if orientation == DirType.VERTICAL:
        ...
        end = int(rect[0] + rect[2] + 1)
    else:
        init = int(rect[1] + 5)
        end = int(rect[1] + rect[3] + 1)
